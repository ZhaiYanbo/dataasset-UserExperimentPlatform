...
function sort(values, ...F) {
  ...
  if (f.length === 1 || F.length > 1) {
    ...
    if (F.length > 1) {
      ...
      index2.sort((i, j) => {
        for (const f2 of F) {
          const c4 = ascending_default(f2[i], f2[j]);
          if (c4)
            return c4;
        }
      });
...
function axis(orient2, scale) {
  var tickArguments = [], tickValues = null, tickFormat2 = null, tickSizeInner = 6, tickSizeOuter = 6, tickPadding = 3, offset = typeof window !== "undefined" && window.devicePixelRatio > 1 ? 0 : 0.5, k2 = orient2 === top || orient2 === left ? -1 : 1, x2 = orient2 === left || orient2 === right ? "x" : "y", transform2 = orient2 === top || orient2 === bottom ? translateX : translateY;
  function axis2(context) {
    ...
    text = text.merge(tickEnter.append("text").attr("fill", "currentColor").attr(x2, k2 * spacing).attr("dy", orient2 === top ? "0em" : orient2 === bottom ? "0.71em" : "0.32em"));
    ...
    tickExit.remove();
    path2.attr("d", orient2 === left || orient2 === right ? tickSizeOuter ? "M" + k2 * tickSizeOuter + "," + range0 + "H" + offset + "V" + range1 + "H" + k2 * tickSizeOuter : "M" + offset + "," + range0 + "V" + range1 : tickSizeOuter ? "M" + range0 + "," + k2 * tickSizeOuter + "V" + offset + "H" + range1 + "V" + k2 * tickSizeOuter : "M" + range0 + "," + offset + "H" + range1);
...
function nodrag_default(view) {
  var root2 = view.document.documentElement, selection = select_default(view).on("dragstart.drag", noevent_default, true);
  if ("onselectstart" in root2) {
    selection.on("selectstart.drag", noevent_default, true);
  } else {
...
function yesdrag(view, noclick) {
  ...
  if (noclick) {
    selection.on("click.drag", noevent_default, true);
    setTimeout(function() {
...
function drag_default() {
  ...
  function mousedowned(event, d) {
    ...
    if (!gesture)
      return;
    select_default(event.view).on("mousemove.drag", mousemoved, true).on("mouseup.drag", mouseupped, true);
...
function brush(dim) {
  var extent = defaultExtent, filter2 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  ...
  brush2.move = function(group2, selection) {
    if (group2.tween) {
    ...
    dx = k2 ? padAngle : tau2 / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups2[i] = { index: i, startAngle: x06, endAngle: x2, value: groupSums[i] };
        } else {
...
Path.prototype = path.prototype = {
  ...
  arcTo: function(x12, y12, x2, y2, r) {
    ...
    if (this._x1 === null) {
      ...
      var x20 = x2 - x06, y20 = y2 - y06, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi3 - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
  ...
  arc: function(x2, y2, r, a0, a1, ccw) {
    ...
    if (da < 0)
      da = da % tau3 + tau3;
    if (da > tauEpsilon) {
      this._ += "A" + r + "," + r + ",0,1," + cw + "," + (x2 - dx) + "," + (y2 - dy) + "A" + r + "," + r + ",0,1," + cw + "," + (this._x1 = x06) + "," + (this._y1 = y06);
    } else if (da > epsilon4) {
...
function ribbon(headRadius) {
  ...
  function ribbon2() {
    var buffer, s2 = source.apply(this, arguments), t = target.apply(this, arguments), ap = padAngle.apply(this, arguments) / 2, argv = slice3.call(arguments), sr = +sourceRadius.apply(this, (argv[0] = s2, argv)), sa0 = startAngle.apply(this, argv) - halfPi2, sa1 = endAngle.apply(this, argv) - halfPi2, tr = +targetRadius.apply(this, (argv[0] = t, argv)), ta0 = startAngle.apply(this, argv) - halfPi2, ta1 = endAngle.apply(this, argv) - halfPi2;
    if (!context)
...
function ringContains(ring, point6) {
  var x2 = point6[0], y2 = point6[1], contains = -1;
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    var fragmentByStart = new Array(), fragmentByEnd = new Array(), x2, y2, t03, t13, t22, t32;
    x2 = y2 = -1;
    t13 = values[0] >= value;
    cases[t13 << 1].forEach(stitch);
    while (++x2 < dx - 1) {
      t03 = t13, t13 = values[x2 + 1] >= value;
      cases[t03 | t13 << 1].forEach(stitch);
    }
    cases[t13 << 0].forEach(stitch);
    while (++y2 < dy - 1) {
      x2 = -1;
      t13 = values[y2 * dx + dx] >= value;
      t22 = values[y2 * dx] >= value;
      cases[t13 << 1 | t22 << 2].forEach(stitch);
      while (++x2 < dx - 1) {
        t03 = t13, t13 = values[y2 * dx + dx + x2 + 1] >= value;
    ...
    function stitch(line) {
      var start2 = [line[0][0] + x2, line[0][1] + y2], end = [line[1][0] + x2, line[1][1] + y2], startIndex = index2(start2), endIndex = index2(end), f, g;
  ...
  function smoothLinear(ring, values, value) {
    ring.forEach(function(point6) {
      var x2 = point6[0], y2 = point6[1], xt = x2 | 0, yt = y2 | 0, v0, v1 = values[yt * dx + xt];
      if (x2 > 0 && x2 < dx && xt === x2) {
        v0 = values[yt * dx + xt - 1];
        point6[0] = x2 + (value - v0) / (v1 - v0) - 0.5;
      }
      if (y2 > 0 && y2 < dy && yt === y2) {
        v0 = values[(yt - 1) * dx + xt];
        point6[1] = y2 + (value - v0) / (v1 - v0) - 0.5;
      }
    });
  }
...
function density_default() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m = dy + o * 2 >> k2, threshold2 = constant_default6(20);
  ...
  density.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default6(+_), density) : x2;
  };
  density.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default6(+_), density) : y2;
  };
...
var Path2 = class {
  ...
  arc(x2, y2, r) {
    ...
    if (!r)
      return;
    this._ += `A${r},${r},0,1,1,${x2 - r},${y2}A${r},${r},0,1,1,${this._x1 = x06},${this._y1 = y06}`;
  }
  rect(x2, y2, w, h) {
    this._ += `M${this._x0 = this._x1 = +x2},${this._y0 = this._y1 = +y2}h${+w}v${+h}h${-w}Z`;
  }
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    while (true) {
      ...
      let x2, y2, c4 = c0 || c1;
      if (c4 & 8)
        x2 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y2 = this.ymax;
      else if (c4 & 4)
        x2 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y2 = this.ymin;
      else if (c4 & 2)
        y2 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x2 = this.xmax;
      else
        y2 = y06 + (y12 - y06) * (this.xmin - x06) / (x12 - x06), x2 = this.xmin;
      if (c0)
        x06 = x2, y06 = y2, c0 = this._regioncode(x06, y06);
      else
  ...
  _project(x06, y06, vx, vy) {
    let t = Infinity, c4, x2, y2;
    if (vy < 0) {
      if (y06 <= this.ymin)
        return null;
      if ((c4 = (this.ymin - y06) / vy) < t)
        y2 = this.ymin, x2 = x06 + (t = c4) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax)
        return null;
      if ((c4 = (this.ymax - y06) / vy) < t)
        y2 = this.ymax, x2 = x06 + (t = c4) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax)
        return null;
      if ((c4 = (this.xmax - x06) / vx) < t)
        x2 = this.xmax, y2 = y06 + (t = c4) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin)
        return null;
      if ((c4 = (this.xmin - x06) / vx) < t)
        x2 = this.xmin, y2 = y06 + (t = c4) * vy;
    }
    return [x2, y2];
  }
  _edgecode(x2, y2) {
...
var Delaunay = class {
  ...
  *neighbors(i) {
  ...
  find(x2, y2, i = 0) {
    ...
    const i0 = i;
    let c4;
    while ((c4 = this._step(i, x2, y2)) >= 0 && c4 !== i && c4 !== i0)
      i = c4;
    return c4;
  }
  _step(i, x2, y2) {
...
function autoType(object2) {
  for (var key in object2) {
    var value = object2[key].trim(), number5, m;
    if (!value)
      value = null;
    else if (value === "true")
      value = true;
...
function centroidRingPoint(lambda, phi2) {
  lambda *= radians2, phi2 *= radians2;
  var cosPhi = cos2(phi2), x2 = cosPhi * cos2(lambda), y2 = cosPhi * sin2(lambda), z = sin2(phi2), cx = y0 * z - z0 * y2, cy = z0 * x2 - x0 * z, cz = x0 * y2 - y0 * x2, m = hypot(cx, cy, cz), w = asin(m), v = m && -w / m;
  X2.add(v * cx);
  Y2.add(v * cy);
  Z2.add(v * cz);
  W1 += w;
...
function circle_default() {
  ...
  function circle2() {
    var c4 = center2.apply(this, arguments), r = radius.apply(this, arguments) * radians2, p = precision.apply(this, arguments) * radians2;
    ring = [];
  ...
  circle2.center = function(_) {
    return arguments.length ? (center2 = typeof _ === "function" ? _ : constant_default7([+_[0], +_[1]]), circle2) : center2;
  };
  circle2.radius = function(_) {
    return arguments.length ? (radius = typeof _ === "function" ? _ : constant_default7(+_), circle2) : radius;
  };
  circle2.precision = function(_) {
    return arguments.length ? (precision = typeof _ === "function" ? _ : constant_default7(+_), circle2) : precision;
  };
  return circle2;
}
...
function pointEqual_default(a2, b) {
  return abs3(a2[0] - b[0]) < epsilon6 && abs3(a2[1] - b[1]) < epsilon6;
}
...
function lengthPoint(lambda, phi2) {
  lambda *= radians2, phi2 *= radians2;
  var sinPhi = sin2(phi2), cosPhi = cos2(phi2), delta = abs3(lambda - lambda03), cosDelta = cos2(delta), sinDelta = sin2(delta), x2 = cosPhi * sinDelta, y2 = cosPhi02 * sinPhi - sinPhi02 * cosPhi * cosDelta, z = sinPhi02 * sinPhi + cosPhi02 * cosPhi * cosDelta;
...
function graticuleX(y06, y12, dy) {
  var y2 = range_default(y06, y12 - epsilon6, dy).concat(y12);
  return function(x2) {
...
function graticuleY(x06, x12, dx) {
  var x2 = range_default(x06, x12 - epsilon6, dx).concat(x12);
  return function(y2) {
...
function interpolate_default2(a2, b) {
  var x06 = a2[0] * radians2, y06 = a2[1] * radians2, x12 = b[0] * radians2, y12 = b[1] * radians2, cy0 = cos2(y06), sy0 = sin2(y06), cy1 = cos2(y12), sy1 = sin2(y12), kx0 = cy0 * cos2(x06), ky0 = cy0 * sin2(x06), kx1 = cy1 * cos2(x12), ky1 = cy1 * sin2(x12), d = 2 * asin(sqrt(haversin(y12 - y06) + cy0 * cy1 * haversin(x12 - x06))), k2 = sin2(d);
  var interpolate = d ? function(t) {
    var B2 = sin2(t *= d) / k2, A5 = sin2(d - t) / k2, x2 = A5 * kx0 + B2 * kx1, y2 = A5 * ky0 + B2 * ky1, z = A5 * sy0 + B2 * sy1;
    return [
      atan2(y2, x2) * degrees3,
      atan2(z, sqrt(x2 * x2 + y2 * y2)) * degrees3
    ];
  } : function() {
  ...
  point: function(x2, y2) {
    switch (this._point) {
      ...
      default: {
        ...
        this._context.arc(x2, y2, this._radius, 0, tau5);
        break;
      }
    }
  },
...
function fitExtent(projection2, extent, object2) {
  return fit(projection2, function(b) {
    var w = extent[1][0] - extent[0][0], h = extent[1][1] - extent[0][1], k2 = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1])), x2 = +extent[0][0] + (w - k2 * (b[1][0] + b[0][0])) / 2, y2 = +extent[0][1] + (h - k2 * (b[1][1] + b[0][1])) / 2;
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b02 + b12, c4 = c0 + c1, m = sqrt(a2 * a2 + b * b + c4 * c4), phi2 = asin(c4 /= m), lambda22 = abs3(abs3(c4) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a2), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
      if (dz * dz / d2 > delta2 || abs3((dx * dx2 + dy * dy2) / d2 - 0.5) > 0.3 || a0 * a1 + b02 * b12 + c0 * c1 < cosMinDistance) {
        resampleLineTo(x06, y06, lambda04, a0, b02, c0, x2, y2, lambda22, a2 /= m, b /= m, c4, depth, stream);
        stream.point(x2, y2);
        resampleLineTo(x2, y2, lambda22, a2, b, c4, x12, y12, lambda12, a1, b12, c1, depth, stream);
      }
    }
  }
  return function(stream) {
...
function projectionMutator(projectAt) {
  var project, k2 = 150, x2 = 480, y2 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default4, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function conicEqualAreaRaw(y06, y12) {
  ...
  function project(x2, y2) {
    var r = sqrt(c4 - 2 * n * sin2(y2)) / n;
    return [r * sin2(x2 *= n), r0 - r * cos2(x2)];
  }
  project.invert = function(x2, y2) {
    var r0y = r0 - y2, l = atan2(x2, abs3(r0y)) * sign(r0y);
    if (r0y * n < 0)
      l -= pi4 * sign(x2) * sign(r0y);
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x2 - 0.307 * k2, y2 + 0.201 * k2]).clipExtent([[x2 - 0.425 * k2 + epsilon6, y2 + 0.12 * k2 + epsilon6], [x2 - 0.214 * k2 - epsilon6, y2 + 0.234 * k2 - epsilon6]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x2 - 0.205 * k2, y2 + 0.212 * k2]).clipExtent([[x2 - 0.214 * k2 + epsilon6, y2 + 0.166 * k2 + epsilon6], [x2 - 0.115 * k2 - epsilon6, y2 + 0.234 * k2 - epsilon6]]).stream(pointStream);
    return reset();
  };
...
function azimuthalInvert(angle2) {
  return function(x2, y2) {
    var z = sqrt(x2 * x2 + y2 * y2), c4 = angle2(z), sc = sin2(c4), cc = cos2(c4);
    return [
      atan2(x2 * sc, z * cc),
      asin(z && y2 * sc / z)
    ];
...
function conicConformalRaw(y06, y12) {
  ...
  project.invert = function(x2, y2) {
    var fy = f - y2, r = sign(n) * sqrt(x2 * x2 + fy * fy), l = atan2(x2, abs3(fy)) * sign(fy);
    if (fy * n < 0)
      l -= pi4 * sign(x2) * sign(fy);
...
function conicEquidistantRaw(y06, y12) {
  ...
  project.invert = function(x2, y2) {
    var gy = g - y2, l = atan2(x2, abs3(gy)) * sign(gy);
    if (gy * n < 0)
      l -= pi4 * sign(x2) * sign(gy);
...
function encloseBasis3(a2, b, c4) {
  var x12 = a2.x, y12 = a2.y, r1 = a2.r, x2 = b.x, y2 = b.y, r2 = b.r, x3 = c4.x, y3 = c4.y, r3 = c4.r, a22 = x12 - x2, a3 = x12 - x3, b22 = y12 - y2, b32 = y12 - y3, c22 = r2 - r1, c32 = r3 - r1, d1 = x12 * x12 + y12 * y12 - r1 * r1, d2 = d1 - x2 * x2 - y2 * y2 + r2 * r2, d3 = d1 - x3 * x3 - y3 * y3 + r3 * r3, ab = a3 * b22 - a22 * b32, xa = (b22 * d3 - b32 * d2) / (ab * 2) - x12, xb = (b32 * c22 - b22 * c32) / ab, ya = (a3 * d2 - a22 * d3) / (ab * 2) - y12, yb = (a22 * c32 - a3 * c22) / ab, A5 = xb * xb + yb * yb - 1, B2 = 2 * (r1 + xa * xb + ya * yb), C2 = xa * xa + ya * ya - r1 * r1, r = -(A5 ? (B2 + Math.sqrt(B2 * B2 - 4 * A5 * C2)) / (2 * A5) : C2 / B2);
  return {
    x: x12 + xa + xb * r,
    y: y12 + ya + yb * r,
    r
...
function place(b, a2, c4) {
  ...
  if (d2) {
    ...
    if (a22 > b22) {
      ...
      c4.x = a2.x + x2 * dx - y2 * dy;
      c4.y = a2.y + x2 * dy + y2 * dx;
    }
  } else {
    c4.x = a2.x + c4.r;
    c4.y = a2.y;
...
function packEnclose(circles) {
  ...
  for (i = 0; i < n; ++i)
    a2 = circles[i], a2.x -= c4.x, a2.y -= c4.y;
  return c4.r;
}
...
function pack_default() {
  ...
  function pack(root2) {
    ...
    if (radius) {
      ...
      root2.eachBefore(radiusLeaf(defaultRadius2)).eachAfter(packChildren(constantZero, 1)).eachAfter(packChildren(padding, root2.r / Math.min(dx, dy))).eachBefore(translateChild(Math.min(dx, dy) / (2 * root2.r)));
    }
    return root2;
  }
  ...
  pack.padding = function(x2) {
    return arguments.length ? (padding = typeof x2 === "function" ? x2 : constant_default8(+x2), pack) : padding;
  };
  return pack;
}
...
function stratify_default() {
  ...
  stratify.id = function(x2) {
    return arguments.length ? (id2 = required(x2), stratify) : id2;
  };
...
function treemap_default() {
  ...
  treemap.paddingInner = function(x2) {
    return arguments.length ? (paddingInner = typeof x2 === "function" ? x2 : constant_default8(+x2), treemap) : paddingInner;
  };
  ...
  treemap.paddingTop = function(x2) {
    return arguments.length ? (paddingTop = typeof x2 === "function" ? x2 : constant_default8(+x2), treemap) : paddingTop;
  };
  treemap.paddingRight = function(x2) {
    return arguments.length ? (paddingRight = typeof x2 === "function" ? x2 : constant_default8(+x2), treemap) : paddingRight;
  };
  treemap.paddingBottom = function(x2) {
    return arguments.length ? (paddingBottom = typeof x2 === "function" ? x2 : constant_default8(+x2), treemap) : paddingBottom;
  };
  treemap.paddingLeft = function(x2) {
    return arguments.length ? (paddingLeft = typeof x2 === "function" ? x2 : constant_default8(+x2), treemap) : paddingLeft;
  };
  return treemap;
}
...
function centroid_default3(polygon) {
  var i = -1, n = polygon.length, x2 = 0, y2 = 0, a2, b = polygon[n - 1], c4, k2 = 0;
  while (++i < n) {
    a2 = b;
    b = polygon[i];
...
function cross_default(a2, b, c4) {
...
function contains_default3(polygon, point6) {
  var n = polygon.length, p = polygon[n - 1], x2 = point6[0], y2 = point6[1], x06 = p[0], y06 = p[1], x12, y12, inside = false;
  for (var i = 0; i < n; ++i) {
    p = polygon[i], x12 = p[0], y12 = p[1];
    if (y12 > y2 !== y06 > y2 && x2 < (x06 - x12) * (y2 - y12) / (y06 - y12) + x12)
      inside = !inside;
    x06 = x12, y06 = y12;
  }
  return inside;
...
function add(tree, x2, y2, d) {
  ...
  while (node.length) {
    if (right2 = x2 >= (xm = (x06 + x12) / 2))
      x06 = xm;
    else
      x12 = xm;
    if (bottom2 = y2 >= (ym = (y06 + y12) / 2))
      y06 = ym;
    else
      y12 = ym;
...
function find_default2(x2, y2, radius) {
  ...
  if (node)
    quads.push(new quad_default(node, x06, y06, x3, y3));
  if (radius == null)
    radius = Infinity;
  else {
  ...
  while (q = quads.pop()) {
    if (!(node = q.node) || (x12 = q.x0) > x3 || (y12 = q.y0) > y3 || (x22 = q.x1) < x06 || (y22 = q.y1) < y06)
      continue;
    if (node.length) {
...
function remove_default2(d) {
  if (isNaN(x2 = +this._x.call(null, d)) || isNaN(y2 = +this._y.call(null, d)))
    return this;
  var parent, node = this._root, retainer, previous, next, x06 = this._x0, y06 = this._y0, x12 = this._x1, y12 = this._y1, x2, y2, xm, ym, right2, bottom2, i, j;
  if (!node)
    return this;
  if (node.length)
    while (true) {
...
function quadtree(nodes, x2, y2) {
  var tree = new Quadtree(x2 == null ? defaultX2 : x2, y2 == null ? defaultY2 : y2, NaN, NaN, NaN, NaN);
...
function transformer2() {
  ...
  scale.invert = function(y2) {
    return clamp(untransform((input || (input = piecewise2(range3, domain.map(transform2), number_default2)))(y2)));
  };
  scale.domain = function(_) {
...
function loggish(transform2) {
  ...
  scale.ticks = function(count3) {
    ...
    if (r = v < u)
      i = u, u = v, v = i;
    var i = logs(u), j = logs(v), p, k2, t, n = count3 == null ? 10 : +count3, z = [];
    if (!(base % 1) && j - i < n) {
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x2 = 1, s2 = 1, t03, t13, t22, k10, k21, interpolator = identity2, transform2, clamp = false, unknown;
  function scale(x3) {
  ...
  scale.domain = function(_) {
    return arguments.length ? ([x06, x12, x2] = _, t03 = transform2(x06 = +x06), t13 = transform2(x12 = +x12), t22 = transform2(x2 = +x2), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1, scale) : [x06, x12, x2];
  };
  scale.clamp = function(_) {
    return arguments.length ? (clamp = !!_, scale) : clamp;
  };
  ...
  return function(t) {
    transform2 = t, t03 = t(x06), t13 = t(x12), t22 = t(x2), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1;
    return scale;
  };
}
function diverging() {
...
var tau6 = 2 * pi5;
function acos2(x2) {
  return x2 > 1 ? 0 : x2 < -1 ? pi5 : Math.acos(x2);
}
function asin2(x2) {
  return x2 >= 1 ? halfPi4 : x2 <= -1 ? -halfPi4 : Math.asin(x2);
}
...
function intersect(x06, y06, x12, y12, x2, y2, x3, y3) {
  var x10 = x12 - x06, y10 = y12 - y06, x32 = x3 - x2, y32 = y3 - y2, t = y32 * x10 - x32 * y10;
  if (t * t < epsilon7)
    return;
  t = (x32 * (y06 - y2) - y32 * (x06 - x2)) / t;
...
function line_default2(x2, y2) {
  ...
  x2 = typeof x2 === "function" ? x2 : x2 === void 0 ? x : constant_default9(x2);
  y2 = typeof y2 === "function" ? y2 : y2 === void 0 ? y : constant_default9(y2);
  function line(data) {
  ...
  line.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default9(+_), line) : x2;
  };
  line.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default9(+_), line) : y2;
  };
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined = constant_default9(true), context = null, curve = linear_default, output = null;
  x06 = typeof x06 === "function" ? x06 : x06 === void 0 ? x : constant_default9(+x06);
  y06 = typeof y06 === "function" ? y06 : y06 === void 0 ? constant_default9(0) : constant_default9(+y06);
  y12 = typeof y12 === "function" ? y12 : y12 === void 0 ? y : constant_default9(+y12);
  function area(data) {
...
function pie_default() {
  var value = identity_default6, sortValues = descending_default2, sort2 = null, startAngle = constant_default9(0), endAngle = constant_default9(tau6), padAngle = constant_default9(0);
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum3 = 0, index2 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
    for (i = 0; i < n; ++i) {
...
function link2(curve) {
  ...
  function link3() {
    var buffer, argv = slice5.call(arguments), s2 = source.apply(this, argv), t = target.apply(this, argv);
    if (!context)
      context = buffer = path_default();
  ...
  link3.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default9(+_), link3) : x2;
  };
  link3.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default9(+_), link3) : y2;
  };
...
var triangle_default = {
  draw: function(context, size) {
    var y2 = -Math.sqrt(size / (sqrt32 * 3));
...
var wye_default = {
  draw: function(context, size) {
    var r = Math.sqrt(size / a), x06 = r / 2, y06 = r * k, x12 = x06, y12 = r * k + r, x2 = -x12, y2 = y12;
    context.moveTo(x06, y06);
    context.lineTo(x12, y12);
    context.lineTo(x2, y2);
...
Bundle.prototype = {
  ...
  lineEnd: function() {
    ...
    if (j > 0) {
      ...
      while (++i <= j) {
        t = i / j;
        this._basis.point(this._beta * x2[i] + (1 - this._beta) * (x06 + t * dx), this._beta * y2[i] + (1 - this._beta) * (y06 + t * dy));
      }
    }
...
function zoom_default2() {
  ...
  zoom.translateBy = function(selection, x2, y2, event) {
    zoom.transform(selection, function() {
  ...
  zoom.translateTo = function(selection, x2, y2, p, event) {
    zoom.transform(selection, function() {
      var e = extent.apply(this, arguments), t = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p;
      return constrain(identity4.translate(p02[0], p02[1]).scale(t.k).translate(typeof x2 === "function" ? -x2.apply(this, arguments) : -x2, typeof y2 === "function" ? -y2.apply(this, arguments) : -y2), e, translateExtent);
    }, p, event);
  };
  ...
  function mousedowned(event, ...args) {
    if (touchending || !filter2.apply(this, arguments))
      return;
    var g = gesture(this, args, true).event(event), v = select_default(event.view).on("mousemove.zoom", mousemoved, true).on("mouseup.zoom", mouseupped, true), p = pointer_default(event, currentTarget), currentTarget = event.currentTarget, x06 = event.clientX, y06 = event.clientY;
    nodrag_default(event.view);
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter2 = typeof _ === "function" ? _ : constant_default10(!!_), zoom) : filter2;
  };
  zoom.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant_default10(!!_), zoom) : touchable;
  };
  zoom.extent = function(_) {
    return arguments.length ? (extent = typeof _ === "function" ? _ : constant_default10([[+_[0][0], +_[0][1]], [+_[1][0], +_[1][1]]]), zoom) : extent;
  };
