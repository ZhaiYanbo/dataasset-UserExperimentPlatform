<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"图骨干计算与分层布局"实验A, {{ secondsToGo }}s
          后将跳转至"图骨干计算与分层布局"实验B
        </p>
        <template #footer>
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'Task_S3-others', // 任务ID
      taskName: '图骨干计算与分层布局实验A', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将给出图谱全貌，请观察图谱结构，统计图中连通子图的数目', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S3-others.png', // 任务介绍图片
      ...
      nextUrl: 'task_S3-ours', // 下一个路由

      // 问卷相关
      questionList: [
        {
          description: '请观察图谱结构，统计图中连通子图的数目',
          caseName: 'Case 1',
          caseDataset: case1,
        },
        {
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S3_others_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns)
      ansObj[ansID] = userAns
      const timeID = `S3_others_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S3_others_indicator_'
