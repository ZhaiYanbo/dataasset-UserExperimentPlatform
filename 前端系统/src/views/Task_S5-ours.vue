<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"全局与局部可调的混合布局"实验B, {{ secondsToGo }}s
          后将跳转至"基本图交互分析技术"实验
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S5/ourGraph'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'task_S5-ours', // 任务ID
      taskName: '图混合布局实验B', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将呈现一个图谱数据，请根据指导员的实验引导，对图谱进行全局和局部布局的切换', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S5-ours.png', // 任务介绍图片
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const timeID = `S5_ours_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S5_ours_indicator_'
