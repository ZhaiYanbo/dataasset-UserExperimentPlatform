    ...
    <div class="interaction-container">
      ...
      <div class="question-container" v-show="isStart">
        ...
        <div class="answer-list" v-show="!isFinish">
          ...
          <a-button class="operate-btn" type="default" @click="clearAns"
            >撤销
          </a-button>
          <a-button
            class="operate-btn"
      ...
      >
        <p>
          恭喜您已完成"基于关系链路的图计算"实验A, {{ secondsToGo }}s
          后将跳转至"基于关系链路的图计算"实验B
        </p>
        <template #footer>
...
// 导入图对象和数据
import OtherGraph from '@/graph-components/Task_S8-1/otherGraph'
...
export default defineComponent({
  ...
  setup() {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskName: '基于关系链路的图计算A', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos:
        '每个小任务将给出一个图谱的全貌，并在图谱中通过红色和蓝色标记两个节点以作为关系链路任务探索的两端节点，其中红色标记的节点为起始节点，蓝色标记的节点为终止节点。请从起始节点出发，依次点击邻居节点以形成一条链路，直至抵达终止节点', // 任务说明文字
        taskIntroImgPath: '../static/task-intro-img/Task_S8-1-others.png', // 任务介绍图片
      ...
      questionList: [
        {
          description:
            '请从起始节点（红色标记的节点）出发，依次点击邻居节点以形成一条链路，直至抵达终止节点（蓝色标记的节点）',
          caseName: 'Case 1',
          caseDataset: case1,
    ...
    const finishedCallback = (ans: any) => {
      reactiveData.userAns = ans
      message.success('您已成功找到一条路径')
    }
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S8_1_others_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns.edges.length)
      ansObj[ansID] = userAns
      const timeID = `S8_1_others_T${reactiveData.curQuestionIndex}_time`
