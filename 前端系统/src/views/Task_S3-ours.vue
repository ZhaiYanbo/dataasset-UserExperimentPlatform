<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"图骨干计算与分层布局"实验B, {{ secondsToGo }}s
          后将跳转至"主结构保持的增量布局"实验A
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S3/ourGraph'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'Task_S3-ours', // 任务ID
      taskName: '图骨干计算与分层布局实验B', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将给出图谱全貌，请观察图谱结构，统计图中连通子图的数目', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S3-ours.png', // 任务介绍图片
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S3_ours_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns)
      ansObj[ansID] = userAns
      const timeID = `S3_ours_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S3_ours_indicator_'
