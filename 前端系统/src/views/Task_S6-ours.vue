<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"基本图交互分析技术"实验, {{ secondsToGo }}s
          后将跳转至"智能化图交互分析导航"实验A
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S6/ourGraph'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'task_S6-ours', // 任务ID
      taskName: '基本图交互分析技术', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将呈现一个图谱数据，请根据指导员的实验引导，对图谱进行基本图交互操作', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S6-ours.png', // 任务介绍图片
      ...
      nextUrl: 'task_S7-others', // 下一个路由

      // 问卷相关
      questionList: [
        {
          description: '请根据指导员的实验引导，对图谱进行基本图交互操作',
          caseName: 'Case 1',
          caseDataset: {
    ...
    const createGraph = () => {
      ...
      // 开启圈选功能
      rawData.graph.callBrushHandle(true)
    }
    ...
    const submitRate = () => {
      ...
      const ansObj = {}
      const preDesc = 'S6_ours_indicator_'
