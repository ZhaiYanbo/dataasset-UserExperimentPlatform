    ...
    <div class="interaction-container">
      ...
      <div class="question-container" v-show="isStart">
        ...
        <div class="answer-list" v-show="!isFinish">
          ...
          <p class="question-info">
            您已选择{{ ansLength }}个 / 共有{{ centralityCount }}个
          </p>
          ...
          <!-- <a-button class="operate-btn" type="default" @click="showCentrality"
            >中心性节点
          </a-button> -->
          <a-button
            class="operate-btn"
      ...
      >
        <p>
          恭喜您已完成"基于中心性的图计算"实验A, {{ secondsToGo }}s
          后将跳转至"基于中心性的图计算"实验B
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S8-3/otherGraph'
...
export default defineComponent({
  ...
  setup() {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskName: '基于中心性的图计算A', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos:
        '每个小任务将给出一个图谱的全貌和该图谱中所含有的中心性节点数量，请观察图谱中各部分结构，根据指导员的实验引导，通过鼠标点击选择您认为的中心性节点，直至所选节点个数达到数量要求', // 任务说明文字
        taskIntroImgPath: '../static/task-intro-img/Task_S8-3-others.png', // 任务介绍图片
      ...
      nextUrl: 'task_S8-3-ours', // 下一个路由

      // 问卷相关
      questionList: [
        {
          description:
            '请观察图谱中各部分结构，根据指导员的实验引导，通过鼠标点击选择您认为的中心性节点',
          caseName: 'Case 1',
          caseDataset: case1,
          centralityCount: case1.nodes.filter((d) => d.properties.isBackbone)
            .length,
        },
        {
          description:
          ...
          caseName: 'Case 2',
          caseDataset: case2,
          centralityCount: case2.nodes.filter((d) => d.properties.isBackbone)
            .length,
        },
        {
          description:
          ...
          caseName: 'Case 3',
          caseDataset: case3,
          centralityCount: case3.nodes.filter((d) => d.properties.isBackbone)
            .length,
        },
      ],
    ...
    const createGraph = () => {
      ...
      rawData.graph = new Graph({
        ...
        callback: (nodes) => {
          reactiveData.userAns = nodes
        },
        updateUserAns: (nodes) => {
          reactiveData.ansLength = nodes.length
        },
        centralityCount: rawData.centralityCount,
      })
    }
    ...
    const startTask = () => {
      ...
      reactiveData.centralityCount = case1.nodes.filter(
        (d) => d.properties.isBackbone
      ).length
      reactiveData.ansLength = 0
      rawData.graphData = toRaw(reactiveData.graphData)
      rawData.centralityCount = case1.nodes.filter(
        (d) => d.properties.isBackbone
      ).length
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S8_3_others_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns)
      ansObj[ansID] = userAns
      const timeID = `S8_3_others_T${reactiveData.curQuestionIndex}_time`
      ...
      reactiveData.centralityCount =
        reactiveData.questionList[
          reactiveData.curQuestionIndex - 1
        ].centralityCount
      rawData.graphData = toRaw(reactiveData.graphData)
      rawData.centralityCount = reactiveData.centralityCount
    ...
    const showCentrality = () => {
      reactiveData.userAns = rawData.graph.showCentrality()
    }
