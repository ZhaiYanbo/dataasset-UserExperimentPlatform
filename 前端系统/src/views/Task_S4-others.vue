<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"主结构保持的增量布局"实验A, {{ secondsToGo }}s
          后将跳转至"主结构保持的增量布局"实验B
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S4/otherGraph'
import {
  getExpandGraphFromNode,
  // getRecommendInitExpandNodes,
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'task_S4-others', // 任务ID
      taskName: '主结构保持的增量布局实验A', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将给出一个初始图谱，请在指导员的实验引导下，对图中的节点进行右键关系扩展，指出图谱的新增结构以及原有结构', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S4-others.png', // 任务介绍图片
      ...
      nextUrl: 'task_S4-ours', // 下一个路由

      // 问卷相关
      questionList: [
        {
          description: '请对图中的节点进行右键关系扩展，并根据指导员的实验引导，判断图谱新增结构以及原有结构',
          caseName: 'Case 1',
          caseDataset: {
    ...
    const initGraphInfos = () => {
      ...
      rawData.container = rawData.selector.slice(1) // 渲染容器ID
      rawData.width = (
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const timeID = `S4_others_T${reactiveData.curQuestionIndex}_time`
      ...
      // 清除先前绘制的元素
      if (document.querySelector('#graph-render canvas')) {
        document.querySelector('#graph-render canvas')?.remove()
      }
      if (document.querySelector('#graph-render .g6-component-contextmenu')) {
        document
          .querySelector('#graph-render .g6-component-contextmenu')
          ?.remove()
      }
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S4_others_indicator_'
