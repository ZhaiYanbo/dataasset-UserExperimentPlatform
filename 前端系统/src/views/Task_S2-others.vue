<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      <div class="question-container" v-show="isStart">
        ...
        <div class="answer-list" v-show="!isFinish">
          ...
          <div class="input-container">
            <a-input-number v-model:value="userAns" placeholder="请输入您的答案"/>
          </div>
          <a-button
      ...
      >
        <p>
          恭喜您已完成"高聚合点边视觉抽象化简"实验A, {{ secondsToGo }}s
          后将跳转至"高聚合点边视觉抽象化简"实验B
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S2/otherGraph'
import case1 from '@/static/dataset/Scenario_2/2_cleaned_graph_N-576_E-639.json'
import case2 from '@/static/dataset/Scenario_2/3_cleaned_graph_N-1095_E-1159.json'
import case3 from '@/static/dataset/Scenario_2/4_cleaned_graph_N-2034_E-2556.json'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'Task_S2-others', // 任务ID
      taskName: '高聚合点边视觉抽象化简实验A', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将给出图谱全貌，请观察图谱中不同的连通子图结构，统计图中节点规模第二大的连通子图内数据表节点的数目', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S2-others.png', // 任务介绍图片
      ...
      nextUrl: 'task_S2-ours', // 下一个路由

      // 问卷相关
      questionList: [
        {
          description: '观察图谱中不同的连通子图结构，统计节点规模第二大的连通子图内数据表节点的数目',
          caseName: 'Case 1',
          caseDataset: case1,
        },
        {
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S2_others_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns)
      ansObj[ansID] = userAns
      const timeID = `S2_others_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S2_others_indicator_'
