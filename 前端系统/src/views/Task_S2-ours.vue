<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"高聚合点边视觉抽象化简"实验B, {{ secondsToGo }}s
          后将跳转至"图骨干计算与分层布局"实验A
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S2/ourGraph'
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'Task_S2-ours', // 任务ID
      taskName: '高聚合点边视觉抽象化简实验B', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将给出图谱全貌，请观察图谱中不同的连通子图结构，统计图中节点规模第二大的连通子图内数据表节点的数目', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S2-ours.png', // 任务介绍图片
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S2_ours_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns)
      ansObj[ansID] = userAns
      const timeID = `S2_ours_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S2_ours_indicator_'
