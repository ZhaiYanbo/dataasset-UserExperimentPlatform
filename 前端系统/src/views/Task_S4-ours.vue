<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"主结构保持的增量布局"实验B, {{ secondsToGo }}s
          后将跳转至"全局与局部可调的混合布局"实验A
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S4/ourGraph'
import {
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'task_S4-ours', // 任务ID
      taskName: '主结构保持的增量布局实验B', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos: '每个小任务将给出一个初始图谱，请在指导员的实验引导下，对图中的节点进行右键关系扩展，指出图谱的新增结构以及原有结构', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S4-ours.png', // 任务介绍图片
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const timeID = `S4_ours_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S4_ours_indicator_'
