<template>
  <div class="page-container">
    <div class="interaction-container">
      ...
      >
        <p>
          恭喜您已完成"增强语义图元视觉编码"实验B, {{ secondsToGo }}s
          后将跳转至"高聚合点边视觉抽象化简"实验A
        </p>
        <template #footer>
...
// 导入图对象和数据
import Graph from '@/graph-components/Task_S1/ourGraph'
...
export default defineComponent({
  ...
  setup() {
    ...
    const reactiveData = reactive({
      // 任务相关
      taskID: 'Task_S1-ours', // 任务ID
      taskName: '增强语义图元视觉编码实验B', // 任务标题
      taskInfosPre: '本实验共3个小任务，任务内容为：', // 任务说明前缀文本
      taskInfos:
        '每一个小任务将给出图谱全貌，请观察图谱节点图元编码，统计当前图谱中节点种类的数目', // 任务说明文字
      taskIntroImgPath: '../static/task-intro-img/Task_S1-ours.png', // 任务介绍图片
    ...
    const submitAns = () => {
      ...
      const ansObj = {}
      const ansID = `S1_ours_T${reactiveData.curQuestionIndex}_ans`
      const userAns = toRaw(reactiveData.userAns)
      ansObj[ansID] = userAns
      const timeID = `S1_ours_T${reactiveData.curQuestionIndex}_time`
    ...
    const submitRate = () => {
      ...
      const name = sessionStorage.getItem('name')
      const ansObj = {}
      const preDesc = 'S1_ours_indicator_'
