...
const renderNodes = (
  ...
  //   graphInstance.nodes.map((d: Node) => ({
  ...
  const legendList = Array.from(
    new Set(graphInstance.nodes.map((d: Node) => d.typeName))
  );
  renderLegend(
    cfg.legendContainer,
    legendList,
  ...
  if (isNewCreated) {
    // 当为新创时则添加 nodesG 元素
    containerEl = graphInstance.container.append('g').attr('class', 'nodesG');
  } else {
    // 否则直接选择（主要用于扩展和收缩时）
    containerEl = graphInstance.container.select('g.nodesG');
  }

  graphInstance.nodesG = containerEl
    .selectAll('g')
    .data(graphInstance.nodes, (d: Node) => d.guid)
    .enter()
    .append('g')
    .attr('id', (d: Node) => `nodeG-${d.guid}`)
    .attr('class', 'nodeG');
  // 超点的根节点 nodesG
  let abstractClusterElements: any;
  let abstractLogicElements: any;

  // 绘制节点
  const normalElements = graphInstance.nodesG;
  ...
  normalElements
    ...
    .attr('transform', (d: Node) => `translate(${0},${cfg.radius / 2})`)
    .text((d: Node) =>
