...
export default class Graph {
  ...
  public selected: Set<Node> = new Set<Node>();
  ...
  renderGraph(): void {
    ...
    for (const community of this.CommunitiesByIdMap.values()) {
      renderCommunityHull(graphInstance, community);
    }
    graphInstance.container.select('.hullsG').attr('opacity', 0);
    // 进行点边事件绑定
    this.bindBaseEvent();
  }
    ...
    if (graphInstance.nodesG) {
      graphInstance.nodesG
        ...
        .on('click', (event: any, d: Node) => {
          ...
          } else if (this.selected.size < this.centralityCount) {
            d.r *= 3;
            this.selected.add(d);
          }

          // 更新用户答案
          this.updateUserAns([...this.selected.values()]);

          if (this.selected.size === this.centralityCount) {
            // 统计点击的为中心性节点的个数
            const ans = [...this.selected.values()].filter(
              (d) => d.properties?.isBackbone
            ).length;
            this.callback(ans);
          } else {
            this.callback(''); // 当用户点满后去掉某个节点，取消答案
          }
          ...
          d3.selectAll('.circle-node')
            .transition()
            .attr('transform', (d: Node) => `scale(${d.r / this.cfg.radius})`);
          d3.selectAll('.circle-node-iconLabel')
            .transition()
            .style(
              'font-size',
              (d: Node) => `${(d.r / this.cfg.radius) * 5}px`
            );
          d3.selectAll('.circle-text')
            .transition()
            .attr(
              'transform',
              (d: Node) => `translate(${0},${d.r + this.cfg.labelPadding})`
            );
          this.GraphInstance.simulation
            .force(
              'collide',
              d3.forceCollide((d: Node) => d.r * 1.5)
            )
            .alphaTarget(0.01)
            .restart();
  ...
  showCommunity(show = true) {
    this.renderSvg.select('.hullsG').attr('opacity', show ? 1 : 0);
  }
  ...
  showCentrality() {
    ...
    d3.selectAll('.circle-node-iconLabel')
      .transition()
      .style('font-size', (d: Node) => `${(d.r / this.cfg.radius) * 5}px`);
    d3.selectAll('.circle-text')
      .transition()
      .attr(
        'transform',
    ...
    this.GraphInstance.simulation
      ...
      .alphaTarget(0.01)
      .restart();
    return this.data.nodes.filter((node) => node.properties.isBackbone);
  }
