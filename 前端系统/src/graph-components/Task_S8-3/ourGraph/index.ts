...
import { renderNodes, renderEdges, renderInit, renderCommunityHull } from './render';
...
export default class Graph {
  ...
  renderGraph(): void {
    ...
    this.data.nodes.forEach((node: Node) => {
      node.r = node.properties.isBackbone
        ? this.radius * 3
        : this.radius;
    });
    ...
    graphInstance.container.select('.hullsG')
      .attr("opacity", 0);
    // 进行点边事件绑定
    this.bindBaseEvent();
  }
