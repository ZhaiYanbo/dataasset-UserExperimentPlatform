...
export default class Graph {
  ...
  // 已选中节点
  public selectedNodes = new Set<Node>();
  // 已选中连边
  public selectedEdges = new Set<Edge>();
  // 可选节点
  public selectableNodes = new Set<Node>();
  // 节点的邻边
  public neighborEdgesByIdMap: any = new Map();
  ...
  setData(data?: any): void {
    ...
    this.data.nodes.forEach((node: Node) => {
      ...
      this.neighborEdgesByIdMap.set(node.guid, []);
    ...
    this.data.edges.forEach((edge: Edge) => {
      ...
      this.neighborEdgesByIdMap.get(edge.sourceID).push({
        source: edge.sourceID,
        edge: edge,
        target: edge.targetID,
      });
      this.neighborEdgesByIdMap.get(edge.targetID).push({
    ...
    if (graphInstance.nodesG) {
      graphInstance.nodesG
        ...
        .on('click', (event: any, d: Node) => {
          ...
          // 如果这个节点可以点击
          if (this.selectableNodes.has(d)) {
            // 高亮节点和连边
            d3.select(`#circle-virtual-background-${d.guid}`)
              .attr('opacity', 1);
            d3.select(`#circle-node-${d.guid}`)
              .attr('opacity', 1);
            const neighborEdges = this.neighborEdgesByIdMap.get(d.guid);
            for (let neighborEdge of neighborEdges) {
              const neighborNode = this.originalNodesByIdMap.get(neighborEdge.target);
              if (this.selectedNodes.has(neighborNode)) {
                d3.select(`#edge-path-${neighborEdge.edge.guid}`)
                  .attr('opacity', 1);
                // 把连边添加进选中集合
                this.selectedEdges.add(neighborEdge.edge);
                break;
              }
            }
            ...
            this.neighborEdgesByIdMap.get(d.guid).forEach(neighborEdge => {
              const neighborNode = this.originalNodesByIdMap.get(neighborEdge.target);
              if (!this.selectedNodes.has(neighborNode)) {
                this.selectableNodes.add(neighborNode);
              }
            });
            // 当到达终点时，调用回调函数
            if (this.selectedNodes.has(this.originalNodesByIdMap.get(this.end)))
  ...
  clearAns() {
    // 清除所有节点及连边高亮
    d3.selectAll('.circle-virtual-background')
      .attr('opacity', 0.2);
    ...
    this.selectableNodes.clear();
    // 高亮起始点
    d3.select(`#circle-virtual-background-${this.start}`)
      .attr('opacity', 1);
    d3.select(`#circle-node-${this.start}`)
      .style('fill', '#ff0000')
      .attr('opacity', 1);
    // 高亮终点
    d3.select(`#circle-virtual-background-${this.end}`)
      .attr('opacity', 1);
    d3.select(`#circle-node-${this.end}`)
      .style('fill', '#0000ff')
      .attr('opacity', 1);
    // 将起点添加到已选择
    this.selectedNodes.add(this.originalNodesByIdMap.get(this.start));
    this.neighborEdgesByIdMap.get(this.start).forEach(neighborEdge => {
      const neighborNode = this.originalNodesByIdMap.get(neighborEdge.target);
      this.selectableNodes.add(neighborNode);
    });
  }
  ...
  clearCache(): void {
    ...
    this.neighborEdgesByIdMap.clear();
