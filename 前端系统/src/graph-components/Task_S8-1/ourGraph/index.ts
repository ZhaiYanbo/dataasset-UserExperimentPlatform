...
export default class Graph {
  ...
  clearAns() {
    // 高亮起始点
    d3.select(`#nodeG-${this.start}`).style("opacity", 1)
    d3.select(`#circle-node-${this.start}`)
      .style('fill', '#ff0000')
    // 高亮终点
    d3.select(`#nodeG-${this.end}`).style("opacity", 1)
  ...
  findPaths() {
    ...
    const source = this.originalNodesByIdMap.get(this.start);
    const target = this.originalNodesByIdMap.get(this.end);

    // console.log("neighborEdgesByIdMap", this.neighborEdgesByIdMap);
    ...
    neighborStack.push(
      this.neighborEdgesByIdMap
        .get(source.guid)
        .filter((edge) => !visited.get(edge.target))
    );
    ...
    while (!mainStack.isEmpty()) {
      ...
      if (mainStack.peek.target === target.guid) {
        ...
        mainStack.toArray().forEach((edge) => {
          ...
          nodes.push(this.originalNodesByIdMap.get(edge.target));
        });
  ...
  showPath(path: any): void {
    d3.selectAll(".nodeG").style("opacity", 0.2);
    d3.selectAll(".linkG").style("opacity", 0.2);
    path.nodes.forEach((d: Node) =>
      d3.select(`#nodeG-${d.guid}`).style("opacity", 1)
    );
    path.edges.forEach((e: Edge) =>
      d3.select(`#linkG-${e.guid}`).style("opacity", 1)
    );
  }
