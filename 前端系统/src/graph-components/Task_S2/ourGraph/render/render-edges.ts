...
const renderEdges = (
  ...
  const edgesArrowTypeList = new Set(
    graphInstance.subEdges.map((d: Edge) => d.relationshipTypeName)
  );
  ...
  if (isNewCreated) {
    ...
    // 否则直接选择（主要用于扩展和收缩时）
    containerEl = graphInstance.subContainer.select('g.linksG');
  }

  // 创建子 linkG 元素
  graphInstance.subLinksG = containerEl
    .selectAll('g')
    .data(graphInstance.subEdges, (d: Edge) => d.guid)
    .enter()
    .append('g')
    .attr('id', (d: Edge) => `linkG-${d.guid}`)
    .attr('class', 'linkG');

  const paths = graphInstance.subLinksG
    .append('path')
