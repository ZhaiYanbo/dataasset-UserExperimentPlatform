...
import { edgesInfoConfig, nodesInfoConfig } from './utils/static-const-values';
import {
  consoleInfos,
  expandRelationShip,
...
export default class Graph {
  ...
  setNodesProps(nodes: Node[]) {
    const newNodes = nodes.map((node: Node) => {
      // 初始化右键扩展的数据结构
      // const relations = Object.values(nodesInfoConfig[node.entity_type].expandRelations);
      node.r = node.r ? node.r : this.radius;
      const relations = Object.values(
        nodesInfoConfig[node.typeName].expandRelations
      );
      ...
      const defNodeProp = {
        ...
        color: nodesInfoConfig[node.typeName].color,
        textLabel: nodesInfoConfig[node.typeName].label,
        expandRelations: nodesInfoConfig[node.typeName].expandRelations,
        isFixed: false,
      };
  ...
  setEdgesProps(edges: Edge[]) {
    ...
    for (const edge of edges) {
      ...
      edge.color = edgesInfoConfig[edge.relationshipTypeName].color;
